  ...
  return (
    <aside className={`${isOpen ? 'w-72' : 'w-20'} bg-slate-900 text-white transition-all duration-300 flex flex-col h-screen border-r border-slate-800`}>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-2 custom-scrollbar">
        {/* Usamos el componente NavItem con la prop 'to' */}
        <NavItem 
        ...
        <NavGroup 
          ...
          subItems={[
            { label: 'Explorar Datos', icon: <FileSpreadsheet size={18} />, to: '/excel' }, // Agregué el link a tu tabla
        ...
        {/* Ruta de Usuarios conectada a /usuarios */}
        <NavItem 
          icon={<Users size={20} />} 
          label="Usuarios" 
          isOpen={isOpen} 
      ...
      <div className="p-4 bg-slate-950/50">
        <button 
          ...
          className="flex items-center gap-3 w-full p-2.5 rounded-lg hover:bg-slate-800 text-slate-400 hover:text-white transition-colors"
        >
          <Menu size={20} />
          {isOpen && <span className="text-sm font-semibold">Cerrar Panel</span>}
        </button>
      </div>
    </aside>
  );
...
// NavItem actualizado para usar Link
...
// NavGroup actualizado para que los subItems también sean Links
